...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 25 02:18:02 2019
...
# draw all results
def draw_boxes(filename, v_boxes, v_labels, v_scores, n):
    # load the image
    ...
    for i in range(len(v_boxes)):
        ...
        cv2.rectangle(img=image,
                      pt1=(int(x1), int(y1)),
                      pt2=(int(x1) + int(width), int(y1) + int(height)),
                      color=(255, 255, 255),
                      thickness=2)
        cv2.putText(image, "{}-{}".format(v_labels[i], round(v_scores[i], 2)), (x1 + (width) // 2, y1),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=1, color=(255, 255, 255), thickness=1, lineType=cv2.LINE_AA)

    cv2.imwrite('samp/pred{}.jpg'.format(n), image)
...
for file in os.listdir('video_frames'):
    l = file.split('.')
    q = l[0].split('e')
    photo_filename = 'video_frames/' + file
    ...
    print(v)
    draw_boxes(photo_filename, v_boxes, v_labels, v_scores, q[1])
    boxes = list()
    v = v + 1
...
num_of_images = len(os.listdir('samp'))

for file in os.listdir('samp'):
...
reader = imageio.get_reader('newVideo.mp4')
...
image_folder = 'samp/'  # make sure to use your folder
video_name = 'room.avi'
...
p = [int(a) for a in p]
p = sorted(p)
images = ['pred' + str(b) + '.jpg' for b in p]
